...
\section{Adjustments to Derivation Tree Element (DTE) Model}\label{sec:adjustments-to-derivation-tree-element-(dte)-model}
In the previous chapter, model for the derivation tree element was introduced.
For the code generation, we extend the class with new methods that help to extract meaningful data from the subtree.
As an example, we show the derivation of the simple of variable declarations\\ \verb+int x; char y+ (Figure \ref{fig:example_subtree})
\begin{figure}[h]
...
\subsubsection{Pebble Game Naive Approach}
...
approach needs to be integrated with the current implementation of the pebble game and, given the recursive structures, it's vital
to keep track of the registers of the corresponding values.
\newpage
...
\newpage
\begin{codeblock}[genereateCodeForCall]
input: FunctionCall call - current function call
output: generated code is added to the main list

generateCodeForCall(FunctionCall call) {
reg := Configuration.getFreeReg()
add new instruction [ sw $RA $reg 0 ]
Configuration.freeRegister(reg)

body := call.function.getBody()
// body -> StS ; RSt
if body.fson is <StS> {
statements = fseq(body.fson)
for all st in statements -> { evalSt(st) } // already shown
body = body.bro.bro // moving pointer to RSt
}
